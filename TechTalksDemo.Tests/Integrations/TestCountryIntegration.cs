...
{
    public class TestCountryIntegration
    {
        private readonly TestServer server;

        public TestCountryIntegration()
        {
        ...
        public async void TestGetById()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var response = await client.GetAsync(string.Format("/api/Countrys/{0}", maxId));
                var result = await response.Content.ReadAsJsonAsync<Country>();                
            ...
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                if(item == null)
                {
                    var newItem = new Country() {
                        Id = 0,
                        Name = string.Format("Country {0}", countBefore)
                    };
                ...
                var response = await client.PostAsJsonAsync("/api/Countrys", item);
                var result = await response.Content.ReadAsJsonAsync<Country>();
        ...
        public async void TestPut()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var response = await client.PutAsJsonAsync(string.Format("/api/Countrys/{0}", itemId), item);
                var result = await response.Content.ReadAsJsonAsync<Country>();
        ...
        public async void TestDelete()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var response = await client.DeleteAsync(string.Format("/api/Countrys/{0}", maxId));
                var result = await response.Content.ReadAsJsonAsync<Country>();
