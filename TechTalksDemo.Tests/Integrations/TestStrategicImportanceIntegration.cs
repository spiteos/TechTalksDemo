...
{
    public class TestStrategicImportanceIntegration
    {
        private readonly TestServer server;

        public TestStrategicImportanceIntegration()
        {
        ...
        public async void TestGetAll()
        {
            ...
            {
                var response = await client.GetAsync("/api/StrategicImportances");
                var result = await response.Content.ReadAsJsonAsync<List<StrategicImportance>>();
        ...
        public async void TestGetById()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var response = await client.GetAsync(string.Format("/api/StrategicImportances/{0}", maxId));
        ...
        public async void TestPut()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var response = await client.PutAsJsonAsync(string.Format("/api/StrategicImportances/{0}", itemId), item);
                var result = await response.Content.ReadAsJsonAsync<StrategicImportance>();
        ...
        public async void TestDelete()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var response = await client.DeleteAsync(string.Format("/api/StrategicImportances/{0}", maxId));
                var result = await response.Content.ReadAsJsonAsync<StrategicImportance>();
