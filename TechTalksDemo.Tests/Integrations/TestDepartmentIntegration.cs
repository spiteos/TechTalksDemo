...
{
    public class TestDepartmentIntegration
    {
        private readonly TestServer server;

        public TestDepartmentIntegration()
        {
        ...
        public async void TestGetAll()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var result = await response.Content.ReadAsJsonAsync<List<Department>>();
        ...
        public async void TestGetById()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var response = await client.GetAsync(string.Format("/api/Departments/{0}", maxId));
        ...
        public async void TestPut()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var response = await client.PutAsJsonAsync(string.Format("/api/Departments/{0}", itemId), item);
                var result = await response.Content.ReadAsJsonAsync<Department>();
        ...
        public async void TestDelete()
        {
            using (var client = server.CreateClient().AcceptJson())
            {
                ...
                var response = await client.DeleteAsync(string.Format("/api/Departments/{0}", maxId));
                var result = await response.Content.ReadAsJsonAsync<Department>();
